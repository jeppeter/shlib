            ...
            while j < len(curcmd):
                ...
                if curch in get_argsopt_needargs_shortopt(args,argsopt,retcmd):
                    if shortstep > 0:
                        ...
                        retcmd='--'
                        write_retfile(resfile,['[%d][%s] multiple need args defined'%(i,curcmd),'please check for options use -h|--help'])
                        break
                    shortstep += 1
...
def basic_common_completion_opt(longopt,args,argsopt,resfile,cmdidx,leftargs,totalargs):
    s = ''
    ...
    if optattr.type == 'int' or optattr.type == 'long':
        ...
        s += all_file_completion(args,argsopt,resfile,cmdidx,leftargs,totalargs)
        return s
    return s
